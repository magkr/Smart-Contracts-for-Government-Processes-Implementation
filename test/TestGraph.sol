...
contract TestGraph {
  ...
  function getVertexFromTitle(bytes32 t) private returns (uint) {
    return _graph.vertxs[_graph.titleToID[t]];
  }
  ...
  function testSetStatus() public {
    ...
    //Assert
    Assert.true(getVertexFromTitle(t).status == st_MARKED, "status is set corretly");
  }
  ...
  function testAddCase() public {
    ...
    //Assert
    Assert.true(getVertexFromTitle(t).status == st_UNDONE, "status for nonroot is set corretly");
    Assert.true(getVertexFromTitle(r).status == st_DONE, "status for root is set corretly");
  }
  ...
  function testIsUndone() public {
    ...
    //Act
    let res1 = Graph.isUndone(_graph, getVertexFromTitle(t), caseID);
    let res2 = Graph.isUndone(_graph, getVertexFromTitle(r), caseID);

    //Assert
    Assert.true(res1, "is UNDONE with for nonroot");
    Assert.false(res2, "is DONE with for root");
  }
  ...
  function testCut() public {
    ...
    //Act
    let res = Graph.cut(arr, newLength);
